...
Provider and model selection are centralized in `config/providers.json`. `REELKIT_IMAGE_TIER=iteration|final` switches the configured fal model without code changes. The real adapter is `scripts/fal_adapter.py`; set `REELKIT_IMAGE_GENERATOR_CMD` to call it and supply `FAL_KEY` only in the process environment from a secure runtime. The key is never written into plans, logs, ledgers, or checkpoints. Each ledger records model/tier and a per-render cost section. Cache keys include the model, so parallel stages can share exact assets without accidental cross-model reuse.

Measured iteration proof (2026-09-13): `fal-ai/fast-sdxl` completed in 1.53 compute seconds at the model page's $0.00125/compute-second price, $0.0019125 total. The second content-addressed pass was an exact cache hit and cost $0. Model page: https://fal.ai/models/fal-ai/fast-sdxl
